...
def create
  ...
  if @kit.save
    redirect_to @kit, notice: "Kit created."
  else
    render :new
  end
end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to kits_path, status: :see_other, notice: "Kit was successfully destroyed." }
  ...
  private
    ...
    def kit_params
      params.expect(kit: [ :name, :description, :user_id ])
      params.require(:kit).permit(:name, :description, item_ids: [])
    end
end
